...
function arrayExecutor(map2, reduce) {
    let { hidden = !1 } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
    return {
        executeSync (node, scope) {
...
const EXECUTORS = {
    ...
    OpCall: {
        async executeAsync (param, scope) {
        ...
        executeSync (param, scope) {
            let { op, left, right } = param;
            const func = operators[op];
            if (!func) throw new Error("Unknown operator: ".concat(op));
            const leftValue = executeSync(left, scope), rightValue = executeSync(right, scope), result = func(leftValue, rightValue);
            if ("then" in result || result.type === "stream") throw new Error("Operator ".concat(op, " not possible in evaluteSync"));
            return result;
        }
    },
    Select: {
    ...
    InRange: mappedExecutor((param)=>{
        let { base, left, right } = param;
        return [
            base,
            left,
            right
        ];
    }, (param, value, leftValue, rightValue)=>{
        let { isInclusive } = param;
    ...
    AccessAttribute: mappedExecutor((param)=>{
        let { base } = param;
        return [
            base || {
                type: "This"
        ...
        let { name } = param;
        return value.type === "object" && value.data.hasOwnProperty(name) ? fromJS(value.data[name]) : NULL_VALUE;
    }),
    AccessElement: mappedExecutor((param)=>{
        let { base } = param;
        return [
            base
        ];
    ...
    Slice: mappedExecutor((param)=>{
        let { base } = param;
        return [
            base
        ];
    }, (param, baseValue)=>{
        let { left, right, isInclusive } = param;
        if (baseValue.type !== "array") return NULL_VALUE;
...
function evaluateQuerySync(tree) {
    let options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
...
array.join.arity = 2;
array.compact = arrayExecutor((param)=>{
    let [array2] = param;
    return {
        array: array2
    };
...
math.avg = arrayReducerExecutor((args)=>({
    ...
    let { count, sum } = param;
    return count === 0 ? NULL_VALUE : fromNumber(sum / count);
});
math.avg.arity = 1;
...
const pt = {};
pt.text = mappedExecutor((args)=>args, function(_, value) {
...
sanity.versionOf = mappedExecutor((param)=>{
    let [value] = param;
    return [
        value,
        {
